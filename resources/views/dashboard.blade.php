<x-app-layout>
  <div class="container mx-auto px-4 py-4">
    <div class="max-w-6xl mb-14 mx-auto tracking-tighter text-center">
      @php
      $isGuest = auth()->guest();
      $heading = $isGuest ? 'Selamat Datang di Readnest' : 'Mau baca apa hari ini, ' . auth()->user()->name . '!';
      $headingClass = $isGuest ? 'text-5xl' : 'text-4xl';
      $paragraphClass = $isGuest ? 'text-xl text-gray-800' : 'text-lg text-gray-600';
      @endphp

      <h1 class="max-w-4xl mx-auto {{ $headingClass }} text-gray-900 mb-4 font-breilga text-6xl">{{ $heading }}</h1>
      <p class="max-w-4xl mx-auto tracking-normal {{ $paragraphClass }} dark:text-gray-600 mb-6 font-jomolhari">
      ...
      <form method="GET" action="{{ route('dashboard') }}" id="search-form" class="flex mb-6 tracking-normal justify-center">
